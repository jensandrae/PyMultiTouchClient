...
class Handler:
    ...
    def scale_by_factor(self, points, factor):
        """Scale a object of points by a given factor. Minimum requirements are three different points. """
        # ToDo: This is not working correct, its just a copy of scale_to_square
        #  Need to test how scale_to_square works !!! Of course we need this function !!!

        bounding = self.bounding_box(points)
        new_points = []
        for point in points:
            qx = point.x * (factor / bounding.width)
            qy = point.y * (factor / bounding.height)
            new_points.append(Point(qx, qy))
        return new_points

    # ToDo: Extended Scaling Function: Eine funktion welche ausgehend von einem Mittelpunkt,
    #  und ausgehend von mindestens 3 außenpunkten diese Außenpunkte um einen faktor x genau
    #  ausgehend vom Mittelpunkt, oder zu diesem hin skaliert !!!
    #  +++ Ein max. schrumpfen festlegen, so das invertierung verhindert wird.
